...
const productCategories = [
  {
    ...
    icon: Pill,
    products: ["Antibiotics", "Pain Relief", "Blood Pressure Meds", "Diabetes Care"],
  },
  {
    id: "wellness",
  ...
  },
  {
    ...
    icon: Leaf,
    products: ["Cough Syrups", "Allergy Relief", "Digestive Aids", "Sleep Support"],
  },
  {
    id: "emergency",
    ...
    icon: AlertCircle,
    products: ["First Aid Kits", "Bandages", "Antiseptics", "Emergency Supplies"],
  },
]
...
export default function Products() {
  ...
  return (
    <section id="products" className="py-20 px-4 sm:px-6 lg:px-8 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            From prescription medications to wellness supplements, we provide everything you need for optimal health.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {productCategories.map((category, index) => {
            ...
            return <ProductCard key={category.id} category={category} Icon={Icon} index={index} />
          })}
        </div>
      </div>
    </section>
...
function ProductCard({
  ...
  Icon,
  index,
}: { category: (typeof productCategories)[0]; Icon: typeof Pill; index: number }) {
  const { ref, isVisible } = useScrollAnimation(0.2)

  return (
    <div ref={ref} className={`animate-on-scroll stagger-${index + 1} ${isVisible ? "visible" : ""}`}>
      <div className="hover-lift border-glow p-8 bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-2xl group cursor-pointer h-full">
